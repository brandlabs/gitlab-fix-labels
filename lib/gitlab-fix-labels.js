...
{
    const ourLabels = (await api.getProjectLabels(toProjectId)).map(l => l.name);
    const dummyLabels = await api.getDummyProjectLabels();

    return Promise.each(dummyLabels, async label =>
    {
        await request.andKeepRetrying(Object.assign({}, requestDefaults,
        {
            method: ourLabels.includes(label.name) ? 'PUT' : 'POST',
